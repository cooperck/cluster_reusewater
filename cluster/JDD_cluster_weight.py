...
a_1 = import_excel_matrix(data_file,6)# 获取测试数据第5个sheet表
#print(a_1)
##测试标准化基础数据导入情况
...
for i in range(len(a_2)):
  a_2[i,13]=abs(a_2[i,13]-1)#pH调整为14-1，0-1，7-0的格式
 #  for j in range(0, 11):
...
def kmeans():
  data=a
  #开始分类
  km =KMeans(n_clusters=4,init='k-means++',algorithm='full')#full-EM算法 'k-means++':启发式选择一个初始聚类中心
...
def dbscan():
  data=a
  #开始分类
  db = DBSCAN(eps = 20, min_samples=3,metric='euclidean')#esp聚类半径，min半径内最少点数，metric距离
...
def hac():
  data = a
  # 开始分类
  ag = AgglomerativeClustering(n_clusters=2, affinity='euclidean',linkage='single')  # affinity距离,linkage连接算法
