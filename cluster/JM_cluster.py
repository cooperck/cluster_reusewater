...
def dbscan():
  data=a
  #开始分类
  db = DBSCAN(eps = 6, min_samples=2,metric='euclidean')#esp聚类半径，min半径内最少点数，metric距离
...
def hac():
  data = a
  # 开始分类
  ag = AgglomerativeClustering(n_clusters=4, affinity='euclidean',linkage='ward')  # affinity距离,linkage连接算法
